# $Id: embed.pm,v 1.13 1996/05/06 20:34:27 dougm Exp $
require 5.002;
...
Typically, an application B<Makefile> will invoke Devel::embed
functions while building your application.  

=head1 @EXPORT

Devel::embed exports the following functions:
...
When invoked as C<`perl -MDevel::embed -e xsinit --`>
...
=head2 Examples
 ...
 perl -MDevel::embed -e xsinit -- -o xsinit.c Socket
...
The code is written to the default file name B<perlxsi.c>.
 ...
 perl -MDevel::embed -e xsinit -- -o xsinit.c -std DBI DBD::Oracle
...
When invoked as C<`perl -MDevel::embed -e ldopts --`>
the following options are recognized:
...
are picked up from the B<extralibs.ld> file in the same directory.
 ...
 perl -MDevel::embed -e ldopts -- -std Socket
...
This will do the same as the above example, along with printing additional arguments for linking with the B<Socket> extension.
 ...
 perl -MDevel::embed -e ldopts -- DynaLoader
...
and B<libperl.a>.
 ...
 perl -MDevel::embed -e ldopts -- -std Msql -- -L/usr/msql/lib -lmsql
